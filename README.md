...
This repository provides an extended and actively maintained version of our artifact archived at [Zenodo](https://zenodo.org/records/15848938).
...
**Objective:**
Show that the finalization process halts completely after the attack launch.
...
```bash
touch 2.txt && docker run -it --rm -v ./2.txt:/app/query/21.txt bsc-attack-2
```

```bash
touch 3.1.txt && docker run -it --rm -v ./3.1.txt:/app/query/21.txt -e DELAY_INTERVAL_MS=25 bsc-attack-3-bootnode
```
